...
client = OpenAI(base_url="http://172.29.15.223:8000/v1", api_key="lm-studio")
...
def contextualize_response(problem, solutions_with_confidences):
    ...
    url = "http://172.29.15.223:8000/v1/chat/completions"
    data = {
    ...
    try:
        ...
        # print("API request successful. Completion:", completion)
    except requests.exceptions.RequestException as e:
        print(f"Error reaching the server: {e}")
        completion = None

    # Process the response (assuming non-streaming for simplicity)
